...
A weakness of the Sweave model is that when you make changes, you have to reprocess the all of the code to reproduce the final LaTeX file. If your analysis is computationally intensive this can take up time. There are some ways around this: designing one's projects so that they are relatively modular is good practice anyway. Two add-on packages for R (\texttt{cacheSweave} and \texttt{weaver}, both available from the R website) are designed to alleviate this problem. 
