...
Why should you bother to do any of this? A common answer is that Science is about doing work that other people can reproduce. What you need to realize, however, is that nearly all of the time the entity that will most need to revisit or  reproduce the work you do is not some Grand Tribunal of Science in the Year 3000 but \emph{your future self}, not too long from now. When you inevitably return to your table or figure or quotation nine months or two years down the line, this future self will have been saved hours trying to reconstruct what it was you thought you were doing and where in the hell you got those coefficients, graphics, tables or quotations from. 
...
\begin{itemize}
	...
	\item Mac OS X runs only on Apple hardware.\footnote{``Hackintosh'' computers exist, most commonly created by people who want to install OS X on a low-end netbook computer. If that's your idea of a good time, go ahead.} Unlike in the past, Apple computers today have the same basic underlying hardware as computers that run Windows. This has two consequences for those considering a move to Mac OS X. First, one can now make direct price comparisons between Apple computers and PC alternatives such as Dells, Lenovos, etc. In general, the more similarly kitted-out a PC is to an Apple machine, the more the price difference between the two goes away.\footnote{Comparisons should still take account of remaining differences in hardware design quality, and of course the OS itself.} However, Apple does not compete at all price-points in the market, so it will always be possible to configure a cheaper PC than one Apple sells, usually at the cost of dropping some set of features. For the same reason, it is also easier to find a PC configuration precisely tailored to some particular set of needs or  preferences --- e.g., with a better display but without some other item --- than may be available from Apple. 
...
These days, I use Mac OS X, and the discussion here reflects that choice. But the other two options are also perfectly viable alternatives. Rather than try to convince you to plump for one option or another, let's look at some applications that will run on all of these operating systems.
...
At this point it's reasonable to ask why I am even mentioning Emacs, let alone recommending you try it. The answer is that, despite its shortcomings, Emacs is nevertheless very, \emph{very} good at managing the typesetting and statistics applications I'm about to discuss. It's so good, in fact, that it has recently become quite popular amongst a set of software developers, pretty much all of whom are much younger than Emacs itself. The upshot is that there has been a run of good, new resources available for learning it quickly and getting it set up a little more easily. These include \href{http://peepcode.com/products/meet-emacs}{Meet Emacs}, a screencast from PeepCode that walks you through the basics of the application, and an associated \href{http://github.com/technomancy/emacs-starter-kit/tree/master}{Emacs Starter Kit} which smooths out Emacs' default settings to make your life much easier.\footnote{I've made some \href{http://kjhealy.github.com/emacs-starter-kit/}{further changes} to this myself, of interest to social-science types.}
